#ifndef LEOGNAN_RANDOMIZER_H
#define LEOGNAN_RANDOMIZER_H
...
namespace leognan {
	/*
	 * A pseudo-random number generator, Complementary-multiply-with-carry generators
	 * 
	 * This implementation is a Complementary-multiply-with-carry generator aka
	 * CMWC generator, described by George Marsaglia
	 */
	...
	class Randomizer {
		...
		static const Setting SettingsList[];

		public:
		enum StateSize {
			 StateSize_8 = 0, // period approx 2^285
			 StateSize_16,    // period approx 2^540
			 StateSize_32,    // period approx 2^1053
			 StateSize_64,    // period approx 2^2077
			 StateSize_128,   // period approx 2^4118
			 StateSize_256,   // period approx 2^8182
			 StateSize_512,   // period approx 2^16410
			 StateSize_1024,  // period approx 2^32794
			 StateSize_4096   // period approx 2^131104
		};



		Randomizer(StateSize size = StateSize_1024) :
			m_array(nullptr),
			m_size(0),
			m_index(0)
		{
			const Setting& setting = SettingsList[size];
			m_multiplier = setting.multiplier;
			m_size = setting.size;

			m_array = new uint32_t[m_size];
		}
		...
		Randomizer&
		operator = (const Randomizer& other) {
		...
		seed(uint32_t value) {
			...
			uint32_t* offset = m_array;
			for(i = m_size, j = value; i != 0; --i, ++offset) {
				j = j ^ (j << 13u);
				j = j ^ (j >> 17u);
				j = j ^ (j << 5u);
				(*offset) = j;
		...
		next() {
			...
			uint64_t t = m_multiplier * m_array[m_index] + m_carry;
			m_carry = (uint32_t)(t >> 32u);

			uint32_t x = (uint32_t)(t + m_carry);
			if (x < m_carry) {
				x++, m_carry++;
			}
		...
		inline double
		next_uniform_in_range(double min, double max) {
			return (max - min) * next_uniform() + min;
		}

		/*
		 * Generate a number according to a Gaussian distribution, using the 
		 * Ziggurat method. Implementation taken from Jochen Voss
		 */
		double
		next_gaussian(double sigma) {
			...
			while(1) {
				...
				if (i < 127) {
					y0 = ytab[i];
					y1 = ytab[i + 1];
					y = y1 + (y0 - y1) * next_uniform();
				} else {
					x = PARAM_R - std::log(1. - next_uniform()) / PARAM_R;
					y = std::exp(-PARAM_R * (x - .5 * PARAM_R)) * next_uniform();
				}
		...
		generate_seed() {
			...
			if (file == nullptr)
				throw std::runtime_error("Unable to open '/dev/urandom' in read-only access");

			if (fread(&value, sizeof(uint32_t), 1, file) != 1)
				throw std::runtime_error("Error while reading 4 bytes from '/dev/urandom'");

			if (fclose(file) != 0)
				throw std::runtime_error("Error while closing '/dev/urandom'");

			return value;
		}

		private:
			static const double ytab[128];        // tabulated values for the heigt of the Ziggurat levels
			static const unsigned long ktab[128]; // tabulated values for 2^24 times x[i]/x[i+1]
			static const double wtab[128];        // tabulated values of 2^{-24}*x[i]
